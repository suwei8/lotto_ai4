...
df.rename(columns={"user_id": "AI-ID", "bai": "百位定1", "shi": "十位定1", "ge": "个位定1"}, inplace=True)
...
def classify(combo: str) -> str:
    ...
    if ordered in ([0, 1, 2], [1, 2, 3], [2, 3, 4], [3, 4, 5], [4, 5, 6], [5, 6, 7], [6, 7, 8], [7, 8, 9]):
        return "顺子"
    return "组六"
...
with columns[0]:
    exclude_types = st.multiselect("排除组选类型", ["组六", "组三", "豹子", "顺子"], key="comb_view_exclude_types")
with columns[1]:
...
with columns[3]:
    remove_permutations = st.checkbox(
        "过滤重复组合（忽略顺序）", key="comb_view_remove_perms"
    )
...
filtered_df = filtered_df[
    (filtered_df["出现次数"] >= selected_range[0])
    & (filtered_df["出现次数"] <= selected_range[1])
]
